...
const RichDropdownMenu: React.FC<RichDropdownMenuProps> = ({
    ...
    return (
        ...
        >
            <button
                className={`flex items-center gap-1 font-medium transition-colors text-[15px] leading-5 cursor-pointer ${isActive || isOpen ? 'text-paystack-blue-hover' : 'text-gray-900 hover:text-paystack-blue-hover'
                    }`}
            >
                {label}
                <FaCaretDown
            ...
            <div className={`absolute left-0 top-[calc(100%+8px)] w-[550px] bg-white rounded-lg shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-gray-100/50 overflow-hidden z-50 flex dropdown-panel-transition ${isOpen ? 'is-open' : ''}`}>
